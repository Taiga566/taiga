...
# ユーザー設定
# =========================================================
INPUT_CSV = "attribute_tags_freetextgemma4test.csv" # 作文が入った新しいCSV
OUTPUT_CSV = "catchcopygemma4togemma4test.csv"
MENUS = ["塩ラーメン", "きつねうどん", "かつ丼"]

SLOGAN_MODEL_ID = "google/gemma-4-31B-it"
#SLOGAN_MODEL_ID = "cyberagent/Llama-3.1-70B-Japanese-Instruct-2407"
...
def _build_user_prompt(description: str, retry: int, gpt_oss: bool) -> str:
    # attribute_summary の代わりに description を直接埋め込む
    base = (
        # 役割
        "あなたはコピーライターです。\n"
...
def _load_slogan_model():
    ...
    if _is_gpt_oss(model_id):
        ...
        if USE_4BIT_FOR_QWEN and not _is_gemma4(model_id):
    ...
    if _is_gemma4(model_id):
        proc = AutoProcessor.from_pretrained(model_id, **tok_kwargs)
        mdl = AutoModelForImageTextToText.from_pretrained(model_id, **mdl_kwargs)
        try:
            if getattr(mdl.generation_config, "pad_token_id", None) is None:
                ...
                mdl.generation_config.pad_token_id = pad_id or eos_id
        except Exception:
            pass
        return proc, mdl
...
def _candidate_lines(gen_text: str) -> List[str]:
    ...
    cand = []
    for ln in lines:
        if COUNT_NOTE_RE.search(ln): continue
        if "analysis" in ln.lower(): continue
        if ASCII_RE.search(ln): continue
        if DIGIT_RE.search(ln): continue
...
def _validate(a: str, b: str, c: str) -> Tuple[bool, str]:
    if not (a and b and c): return False, "missing_lines"
    if not (_len_ok(a) and _len_ok(b) and _len_ok(c)): return False, "length_not_8_12"
    if ASCII_RE.search(a+b+c) or DIGIT_RE.search(a+b+c): return False, "contains_ascii_or_digit"
    return True, ""
...
def main():
    ...
    for i, r in df.iterrows():
        ...
        if VERBOSE:
            # 表示が長くなるためdescriptionは冒頭30文字のみ表示
